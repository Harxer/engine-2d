...
export default class Layout {
  ...
  insertContextHole(vertices) {
    ...
    if (this.meshContext === undefined) {
      ...
      if (globalDebug) console.log(`Creating root boundary.`, [polygon])
      return this.meshContext;
    }
    if (globalDebug) console.log(`Adding polygon.`, [polygon])
  ...
  contextSelection(p) {
    ...
    if (holeSelected) {
      ...
      if (globalDebug) console.log(`Selected hole`, [this.meshContext.bounds])
    } else {
      if (this.meshContext.bounds.containsPoint(p)) {
        if (this.meshContext.parent !== undefined) {
          this.meshContext = this.meshContext.parent;
          if (globalDebug) console.log(`Selected bounds`, [this.meshContext.bounds])
        }
      } else {
        let graphTriangle = this.meshContext.triangulatedGraph.find(({triangle}) => triangle.containsPoint(p));
        if (graphTriangle !== undefined) {
          if (globalDebug) console.log(`Graph triangle ${graphTriangle.triangle.console.logString()}`, [graphTriangle.triangle])
        }
      }
    }
  }
