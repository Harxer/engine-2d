import { Segment, Vector, Polygon, orientation, equals } from '@harxer/geometry'
...
export default function triangulateGraph(boundsPolygon, holePolygons) {
  if (globalDebug) console.log('Generating triangulation.', holePolygons, true)
  ...
  const setNodeInnerAngle = (i) => {
    ...
    if (globalDebug) console.log(`  Angle node compute ${node.angle / Math.PI * 180}.`, [new Segment(v, v.copy.add(prevVector)), new Segment(v, v.copy.add(nextVector))])
    return node;
  }
  ...
  const setNodeData = (_, i) => {
    ...
    if (equals(cross + largestAbsValue, largestAbsValue) || cross <= 0 || Number.isNaN(cross)) {
      node.convex = false;
      node.eartip = false;
      if (globalDebug) console.log(`    Setting node data CROSS:${node.convex}`, [vPrev, v, vNext])
      return
    }

    if (globalDebug) console.log(`    Setting node data. ${cross} from ${largestAbsValue} Cross: ${equals(cross + largestAbsValue, largestAbsValue)}, ori: ${orientation(vNext, v, vPrev)}`, [new Segment(vPrev, v), new Segment(v, vNext)])
    ...
    if (globalDebug) console.log(`      ...EAR: ${node.eartip}`, [new Segment(vPrev, v), new Segment(v, vNext)])
  }
  ...
  while (holePolygonsRemaining.length > 0) {
    ...
    if (bridgeEdge === undefined) {
      ...
      if (globalDebug) console.log('  No bridge edges. Pushing to end.')
      // Push hole to end
      holePolygonsRemaining.push(hole);
      if (globalDebug) console.log('  Remaining new triangulation.', holePolygonsRemaining)
      continue;
    };

    if (globalDebug) console.log(`Bridge edge generated ${bridgeEdge.logString()} at ${iHoleVertex}`, [bridgeEdge])
    ...
    const bridgeInnerAngle = function(i) {
      ...
      if (globalDebug) console.log(`  Angle bridge compute ${angle / Math.PI * 180}`, [new Segment(v, v.copy.add(prevVector)), new Segment(v, v.copy.add(nextVector))])
      return angle;
    }
      ...
      .filter(({vertex}, iNode) => {
        if (bridgeEdge.a.equals(vertex)) {
          if (bridgeInnerAngle(iNode) > setNodeInnerAngle(iNode).angle) {
            if (globalDebug) console.log(`     Excluding vertex ${bridgeInnerAngle(iNode) * 180 / Math.PI} vs ${setNodeInnerAngle(iNode).angle * 180 / Math.PI}`, [vertex])
            return false
          }
          return true;
        }
    ...
    if (globalDebug) console.log(`First overlap index ${boundsVertexEntry}`, [graphBuilder[boundsVertexEntry].vertex]);
    if (overlappingNodes.length > 1) {
      ...
      if (globalDebug) console.log(`Insertion ${boundsVertexEntry}`, overlappingNodes.map(node => new Segment(graphBuilder[graphBuilder.indexOf(node)].vertex, graphBuilder[graphBuilder.indexOf(node) + 1].vertex)));
    }
  ...
  while (triangles.length < n - 2) {
    ...
    if (node === undefined) {
      if (globalDebug) console.log('No eartips.');
      break;
    };
    ...
    if (globalDebug) console.log(`Processing triangle at ${iNode}`, [triangle])
  ...
  if (globalDebug) console.log(`Created triangles: ${triangles.length}.`, triangles);
  return graph;
}
